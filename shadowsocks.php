...
function getConfigurationFile($params, $port)
{
  $node = explode("|",str_replace(array("\r", "\n"), "", $params['configoption4']));
  $password = $params['password'];
  	$string='{
          	"index" : 0,
		"random" : false,
  ...
  for($i=0;$i<$count;)
  {
    ...
    $cname = $node[$i];$i++;
    $method = $node[$i];$i++;
    array_push($temparray,array("remarks"=>$name,
			"server"=>$cname,
			"server_port"=>$port,
                              "method"=>$method,
                              "obfs"=>"plain",
			"obfsparam"=>"",
                              "remarks_base64"=>base64_encode($name),
                              "password"=>$password,
                              "tcp_over_udp"=>false,
                              "udp_over_tcp"=>false,
                              "group"=>"NeWorld.win",
...
function shadowsocks_ClientArea($params){
  ...
  //file_put_contents('/home/wwwroot/default/whmcs/atestc', date("Y-m-d H:i:s").$serveraccesshash."\n");
  ...
  $traffic_free = ($traffic * 1024 - ($traffic_upload + $traffic_download));
  ...
  if(empty($password))
  {
    while($x <= $count){
      ...
      $cname = '当前服务未开通';$x++;
      $method = '当前服务未开通';$x++;
      $ssqrcode = base64_encode($method.':'.$password.'@'.$cname.':'.$query_data['port']);
      $node_list .= "<tr>
  ...
  $json_data=getConfigurationFile($params, $query_data['port']);
  
  $download_json="<div style=\"text-justify:distribute-all-lines;color:#999;border-color:#E9E9E9;padding:10px;border-radius:4px;margin:5px 0;border:1px solid 			#eee;font-size:12px\"><a name=\"dowonload_json\" download=\"gui-config.json\" href='data:application/json;charset=utf-8,{$json_data}'>点此下载Windows客户端所有节点配置json文件</a> </div>";
